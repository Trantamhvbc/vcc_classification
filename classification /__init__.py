...
def extraFeature(data,model):
    ...
    pickle.dump(model, open('model_w2v_128', 'wb'))
    pad = []
    for i in range(256):
        pad.append(0)
    ret = []
...
def build_model(n_class, input_dim):
    ...
    adam = optimizers.Adam(lr=0.001,beta_1=0.9, beta_2=0.999, amsgrad=False)  #l0.001, beta_1=0.9, beta_2=0.999, amsgrad=False)
...
if __name__ == '__main__':
    print("loading data ...")
    dataset,labelset = readFileText("./data/train.txt")
    print("load data done")
    # convert = []
    # max  = 0
    # for i in dataset:
    ...
    print("Feature data raw to vector 17*32 dim")
    model = Word2Vec.load('word2vec.model')
    data = extraFeature(dataset,model)
    min_max_scaler = preprocessing.MinMaxScaler()
    min_max_scaler.fit(data)
    data = min_max_scaler.transform(data)
    print("save done")
    ...
    label = oneHotCoding(labelset)
    print(label[0])
    x_train =  np.array( data[0 : int( 4*len(data) / 5)] )
    x_test = np.array( data[int(4 * len(data) / 5):len(data)] )
    y_train = np.array( label[0: int(4 * len(data) / 5)] )
    y_test = np.array( label[int(4 * len(data) / 5):len(data)] )
    epochs = 128
